...
- **Risk:** `visitToTimes` stores the literal wall-clock time as a `Z` ISO (no tz conversion) to match how existing appts are stored/displayed; if the existing calendar view applies tz math this may shift display — verify in T5 manual smoke and adjust if Cris's times render off.
