...
async function sendCancellationEmail(appt) {
  ...
  if (appt.customer_email && !alreadySent('appt_cancel', appt.id, 'customer')) {
    const lead = `Hi ${escHtml(firstName(appt.customer_name))}, your scheduled appointment has been <strong>removed from the Bark &amp; Stroll calendar</strong>. Please reach out if this was a mistake and you need to re-add your pup to our calendar.`;
    const rows = [
  ...
  if (appt.employee_email && !alreadySent('appt_cancel', appt.id, 'employee')) {
    const lead = `Hi ${escHtml(firstName(appt.employee_name))}, <strong>this visit has been cancelled — you do not need to go.</strong>`;
    const rows = [
      ['Date', dateStr, 'font-weight:600'],
      ['Time', `${timeStr} to ${endStr}`, 'font-weight:600'],
      ['Service', appt.service_name],
...
async function sendRequestNotification(request) {
  ...
  const html = `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:500px;margin:0 auto">
      ...
      <div style="background:#ffffff;padding:24px;border:1px solid #e8e8e5;border-top:none;border-radius:0 0 12px 12px">
        <table style="width:100%;font-size:14px;border-collapse:collapse">
          <tr><td style="padding:8px 0;color:#888;width:100px">Client</td><td style="padding:8px 0;font-weight:600">${escHtml(request.customer_name)}</td></tr>
          <tr><td style="padding:8px 0;color:#888">Dog${request.dogs && request.dogs.length > 1 ? 's' : ''}</td><td style="padding:8px 0">${escHtml(request.dog_names)}</td></tr>
          <tr><td style="padding:8px 0;color:#888">Service</td><td style="padding:8px 0">${escHtml(request.service_name)}</td></tr>
          ...
          ${request.notes ? `<tr><td style="padding:8px 0;color:#888">Notes</td><td style="padding:8px 0">${escHtml(request.notes)}</td></tr>` : ''}
        </table>
        <div style="text-align:center;margin-top:20px">
...
module.exports = { sendAppointmentEmail, sendBatchAppointmentEmail, sendCancellationEmail, sendRequestNotification, sendContactFormNotification, sendApplicantNotification, sendApplicantFinalistDisclosure, sendApplicantCheckrHeadsUp, sendApplicantContractorAgreement, __setTransporter };
